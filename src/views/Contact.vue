<template>
	...
	<form @submit="onSubmit" :disabled="isSubmitting">
		...
		<div class="relative mb-4 w-[302px] h-[67px]" :class="{'border border-red-800': !!errors.captchaToken}">
			<Turnstile :site-key="siteKey" v-model="captchaToken" v-bind="captchaTokenAttrs" @expired="handleExpire" />
			<p class="text-[10px] text-red-800 absolute top-1 left-4">{{ errors.captchaToken }}</p>
		</div>
		<button class="group relative flex justify-center items-center w-[195px]  cursor-pointer rounded bg-primary p-3 text-white fill-white transition duration-300
			...
			<span v-if="!isSubmitting" class="relative group-hover:-translate-x-6 transition-all duration-300">Enviar Mensagem</span>
			
			
			<svg v-if="isSubmitting" class="h-4 v-4 mr-3 animate-spin translate-x-[155px] relative" aria-hidden="true" role="status"  viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
			...
			<span v-if="isSubmitting" class="relative -translate-x-[60px] w-[142px] ">Enviando...</span>
			
		</button>

		<div class="absolute bg-gray-900/80  top-0 left-0 w-full h-full shadow flex justify-center items-center z-50" v-if="alert.show">
			<div class="w-2/3 p-5 bg-white rounded flex" v-if="alert.success">
			...
			<div class="w-2/3 p-5 bg-white rounded flex" v-else>
